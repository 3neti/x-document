...
| Browser projection | Read-only `browser/1.0` DTO model, builder, schema/manifest, deterministic IDs, canonical/display values, and vendor JSON driver output | Three-fixture semantic, schema, identity, ordering, and negative tests | JSON only; no HTML, UI, localization, editing, or action execution | Implemented | Add HTML only through a separate adapter |
